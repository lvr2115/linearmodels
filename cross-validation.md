...
This model chases too much. Future data will not have the same outliers
that make this wiggle model fit.
...
\[1\] 0.8509912 - linear \[1\] 0.3594032 - smooth \[1\] 0.4281895 -
wiggle
...
(The lower the RMSE, the better it fits). However, to get a sense of
model stability we really need to iterate this whole process. Of course,
this could be done using loops but that’s a hassle …
...
However, it’s not compatible with ‘gam’, so we have to convert each
training and testing dataset (and lose that nice memory-saving stuff
that ‘crossv_mc’ does in the process) using the code below. It’s worth
...
Is it possible for me to fit the linear model to every element in my
training column?
...
(The .x takes the first training datasets and plugs it there so its
mapping across every train dataset.)
...
What you have is the dist of RMSE across a 100 training/testing splits
fitting the linear model, smooth and wiggle models.
...
differences between the piecewise and gam fits are small enough that the
easy interpretation of the piecewise model “wins”. “comparing the slopes
of those 2 lines in the PW model is easier to interpret than the smooth
line”
